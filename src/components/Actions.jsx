...
const Actions = ({ setOpenSidebar = () => { }, handleDeleteModalOpen = () => { }, setForPriority = () => { }, setSideBarTitle = () => { }, setForAgent = () => { }, actionTime, onRefresh = () => { }, disabled = false }) => {
    return (
        <Grid item xs={12} lg={12} marginTop={1} display={"flex"} alignItems={"center"} justifyContent={"space-between"} paddingBottom={0} marginBottom={0} paddingLeft={0} paddingRight={0}>
            <Box display={"flex"} alignItems={"center"} alignContent={"center"} gap={1}>
                <Button color='default' disabled={disabled} className='actionBtn' onClick={() => {
                    handleDeleteModalOpen()
                }} startIcon={<DeleteOutlinedIcon className='actionIcon' fontSize="large" />}>Delete</Button>
                <Button disabled={disabled} color='default' className='actionBtn' onClick={() => {
                    setForAgent(false)
                    setForPriority(true)
            ...
            <Box display={"flex"} alignItems={"center"} alignContent={"center"} gap={2}>
                <Typography sx={{ fontFamily: "Inter", fontSize: "14px", color: "#394762", lineHeight: "21px", fontWeight: "400" }} variant="filterText" marginBottom={0} gutterBottom>{actionTime ?? ""}</Typography>
